    ...
    const actions = {
        ...
        processPayment: (pid: string, status: string) => {
            ...
            if (status === 'APPROVED' && user) {
                ...
                updates[`users/${user.id}/balance`] = pay.type === 'DEPOSIT' ? Number(user.balance) + amount : Number(user.balance) - amount;
            }
...
const Layout = ({ children }: any) => {
    ...
    return (
        <div className="min-h-screen bg-[#020617] text-slate-200 font-['Hind_Siliguri'] pb-20 selection:bg-orange-500/30">
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 flex justify-around py-4 z-50 rounded-t-[32px] shadow-2xl">
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4">
                {tournaments.slice().sort((a:any, b:any) => b.startTime - a.startTime).map((t: any) => {
                    ...
                    return (
                        <div key={t.id} onClick={() => navigate(`/match/${t.id}`)} className="bg-slate-900/50 border border-slate-800 p-6 rounded-[32px] hover:border-orange-500/50 transition-all cursor-pointer shadow-xl group">
                            ...
                            <h4 className="font-black text-lg text-slate-100 uppercase group-hover:text-orange-500 transition-colors">{t.title}</h4>
                ...
                {!tournaments.length && <p className="text-center py-20 opacity-30 text-xs italic uppercase font-black">কোনো টুর্নামেন্ট নেই</p>}
            </div>
        </div>
    );
};
                ...
                {isJoined && status.isLive ? (
                    <div className="bg-green-600/10 border-2 border-green-500/50 p-8 rounded-[32px] text-center space-y-4 mb-6 shadow-xl animate-in zoom-in">
                        ...
                        <div className="space-y-2">
                            <p className="text-white font-black text-2xl tracking-widest">ID: {t.roomId || 'অপেক্ষা করুন...'}</p>
                            <p className="text-white font-black text-2xl tracking-widest">PASS: {t.password || 'অপেক্ষা করুন...'}</p>
                        </div>
                    </div>
                ) : isJoined ? (
                    ...
                    status.label !== 'Completed' && (
                        <div className="space-y-6 mb-8">
                            ...
                            <button onClick={handleJoin} className="w-full bg-orange-600 py-5 rounded-[24px] font-black text-sm uppercase text-white shadow-xl active:scale-95 transition-all">জয়েন করুন (৳{totalFee})</button>
                        </div>
                    )
                )}
...
const Wallet = () => {
    ...
    if (!currentUser) return <div className="p-20 text-center text-xs font-black uppercase tracking-widest">লগইন করুন</div>;
        ...
        <div className="space-y-6">
            ...
            <div className="flex bg-slate-900 p-1.5 rounded-[24px] border border-slate-800 shadow-xl">
                {['DEPOSIT', 'WITHDRAW', 'HISTORY'].map(t => (
                    <button key={t} onClick={() => setTab(t)} className={`flex-1 py-4 text-[10px] font-black rounded-2xl transition-all ${tab === t ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-500'}`}>{t}</button>
                ))}
            </div>
            {tab !== 'HISTORY' ? (
                <form onSubmit={handleSubmit} className="bg-slate-900 p-8 rounded-[40px] border border-slate-800 space-y-6 shadow-2xl">
                    ...
                    {tab === 'DEPOSIT' && (
                        <div className="p-6 bg-slate-950 rounded-3xl border border-orange-500/10 text-[10px] font-black text-orange-400 leading-relaxed shadow-inner">
                            নিচের যেকোনো নাম্বারে সেন্ডমানি করে আবেদন করুন:<br/><br/>
                    ...
                    <div className="space-y-4">
                        <input type="number" placeholder="টাকার পরিমাণ (৳)" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white outline-none focus:border-orange-500" required />
                        <input type="text" placeholder="বিকাশ/নগদ নাম্বার" value={form.num} onChange={e => setForm({...form, num: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white outline-none focus:border-orange-500" required />
                        {tab === 'DEPOSIT' && <input type="text" placeholder="ট্রানজেকশন আইডি" value={form.trx} onChange={e => setForm({...form, trx: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white outline-none focus:border-orange-500" required />}
                    </div>
                    <button className="w-full bg-orange-600 py-5 rounded-[24px] font-black text-xs uppercase text-white shadow-xl">নিশ্চিত করুন</button>
                </form>
            ) : (
                <div className="space-y-3">
                    {payments.filter((p:any)=>p.userId === currentUser.id).sort((a:any,b:any)=>b.timestamp-a.timestamp).map((p: any) => (
                        <div key={p.id} className="bg-slate-900 p-5 rounded-[28px] border border-slate-800 flex justify-between items-center shadow-lg">
                            <div>
                            ...
                            <p className="text-lg font-black text-orange-500">৳{p.amount}</p>
                        </div>
                    ))}
                </div>
            )}
...
const Admin = () => {
    const { payments, users, tournaments, settings, processPayment, addTournament, removeTournament, updateTournament, setSettings, updateUserBalance, sendMessage, messages } = useStore();
        ...
        <div className="space-y-6">
            <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
                {['PAYMENTS', 'MATCHES', 'USERS', 'SUPPORT', 'SETTINGS'].map(t => (
                    <button key={t} onClick={() => setTab(t)} className={`px-5 py-3 rounded-2xl text-[9px] font-black uppercase border whitespace-nowrap transition-all ${tab === t ? 'bg-orange-600 text-white border-orange-500 shadow-lg' : 'bg-slate-900 text-slate-500 border-slate-800'}`}>{t}</button>
                ))}
            </div>
            ...
            {tab === 'PAYMENTS' && (
                <div className="space-y-4">
                    ...
                    {!payments.filter((p:any)=>p.status==='PENDING').length && <p className="text-center py-20 opacity-30 text-xs italic uppercase font-black">কোনো পেন্ডিং নেই</p>}
                </div>
            )}
            ...
            {tab === 'MATCHES' && (
                <div className="space-y-4">
                    <div className="bg-slate-900 p-8 rounded-[40px] border border-orange-500/50 space-y-4 shadow-2xl">
                        ...
                        <input placeholder="টাইটেল" value={newT.title} onChange={e=>setNewT({...newT, title: e.target.value})} className="w-full bg-slate-950 p-4 rounded-xl text-xs font-black border border-slate-800 text-white outline-none" />
                        
                        <div className="grid grid-cols-3 gap-2">
                            <div className="space-y-1">
                                <label className="text-[8px] text-slate-500 font-black ml-1">১ম প্রাইজ</label>
                                <input type="number" value={newT.prizes.first} onChange={e=>setNewT({...newT, prizes: {...newT.prizes, first: Number(e.target.value)}})} className="bg-slate-950 p-3 rounded-xl text-[10px] font-black border border-slate-800 text-white" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[8px] text-slate-500 font-black ml-1">২য় প্রাইজ</label>
                                <input type="number" value={newT.prizes.second} onChange={e=>setNewT({...newT, prizes: {...newT.prizes, second: Number(e.target.value)}})} className="bg-slate-950 p-3 rounded-xl text-[10px] font-black border border-slate-800 text-white" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[8px] text-slate-500 font-black ml-1">৩য় প্রাইজ</label>
                                <input type="number" value={newT.prizes.third} onChange={e=>setNewT({...newT, prizes: {...newT.prizes, third: Number(e.target.value)}})} className="bg-slate-950 p-3 rounded-xl text-[10px] font-black border border-slate-800 text-white" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <input type="number" placeholder="বেস এন্ট্রি ফি (৳)" value={newT.entryFee} onChange={e=>setNewT({...newT, entryFee: Number(e.target.value)})} className="bg-slate-950 p-4 rounded-xl text-xs font-black border border-slate-800 text-white outline-none" />
                            <input type="number" placeholder="পার কিল (৳)" value={newT.perKill} onChange={e=>setNewT({...newT, perKill: Number(e.target.value)})} className="bg-slate-950 p-4 rounded-xl text-xs font-black border border-slate-800 text-white outline-none" />
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <input type="datetime-local" onChange={e=>setNewT({...newT, startTime: new Date(e.target.value).getTime()})} className="bg-slate-950 p-4 rounded-xl text-[10px] font-black border border-slate-800 text-white outline-none" />
                            <input placeholder="রুম আইডি" value={newT.roomId} onChange={e=>setNewT({...newT, roomId: e.target.value})} className="bg-slate-950 p-4 rounded-xl text-[10px] font-black border border-slate-800 text-white" />
                        </div>
                        <input placeholder="পাসওয়ার্ড" value={newT.password} onChange={e=>setNewT({...newT, password: e.target.value})} className="w-full bg-slate-950 p-4 rounded-xl text-[10px] font-black border border-slate-800 text-white" />
                        
                        <button onClick={handleSaveTournament} className="w-full bg-orange-600 py-4 rounded-2xl font-black text-[10px] uppercase text-white shadow-xl">ম্যাচ সেভ করুন</button>
                    </div>
                    <div className="grid gap-3">
                        {tournaments.map((t: any) => (
                            <div key={t.id} className="bg-slate-900 p-5 rounded-[24px] border border-slate-800 flex justify-between items-center shadow-lg">
            ...
            {tab === 'USERS' && (
                <div className="space-y-3">
                    {users.map((u: any) => (
                        <div key={u.id} className="bg-slate-900 p-6 rounded-[32px] border border-slate-800 flex justify-between items-center shadow-lg">
                            <div>
                            ...
                            <button onClick={() => { 
                                ...
                                if(val !== null) updateUserBalance(u.id, Number(val)).then(() => alert('সফল হয়েছে!'));
            ...
            {tab === 'SUPPORT' && (
                <div className="space-y-4">
                    {chatWith ? (
                        <div className="bg-slate-900 rounded-[32px] flex flex-col h-[450px] border border-slate-800 overflow-hidden shadow-2xl">
                            <div className="p-4 bg-slate-950 font-black text-[10px] flex justify-between items-center border-b border-slate-800 uppercase text-orange-500 tracking-widest">
                                চ্যাট: {chatWith.name}
                                <button onClick={() => setChatWith(null)} className="text-slate-500">✕</button>
                            </div>
                            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-950/20 scrollbar-hide">
                                {messages.filter((m: any) => m.senderId === chatWith.id || m.receiverId === chatWith.id).sort((a:any,b:any)=>a.timestamp-b.timestamp).map((m: any) => (
                                    <div key={m.id} className={`flex ${m.senderId === 'admin-master' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[80%] px-5 py-3 rounded-2xl text-[10px] font-bold ${m.senderId === 'admin-master' ? 'bg-orange-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-300 rounded-tl-none'}`}>{m.message}</div>
                                    </div>
                                ))}
                            </div>
                            <form onSubmit={(e)=>{e.preventDefault(); if(reply.trim()) { sendMessage(reply, chatWith.id); setReply(''); }}} className="p-4 bg-slate-950 flex gap-2 border-t border-slate-800">
                                <input value={reply} onChange={e => setReply(e.target.value)} placeholder="রিপ্লাই দিন..." className="flex-1 bg-slate-900 p-4 rounded-2xl text-xs font-black border border-slate-800 text-white outline-none focus:border-orange-500" />
                                <button type="submit" className="bg-orange-600 px-8 rounded-2xl font-black text-[10px] uppercase text-white">পাঠান</button>
                            </form>
                        </div>
                    ) : (
                        <div className="grid gap-2">
                            {Array.from(new Set(messages.map((m:any) => m.senderId === 'admin-master' ? m.receiverId : m.senderId))).map(uid => {
                                ...
                                return (
                                    <div key={u.id} onClick={() => setChatWith(u)} className="bg-slate-900 p-5 rounded-[24px] text-[11px] font-black cursor-pointer border border-slate-800 hover:border-orange-500 transition-all flex justify-between items-center group">
                                        <span className="uppercase">{u.name} ({u.phone})</span>
            ...
            {tab === 'SETTINGS' && (
                <div className="bg-slate-900 p-8 rounded-[40px] border border-slate-800 space-y-6 shadow-2xl">
                    <h4 className="font-black text-[10px] uppercase text-orange-500 tracking-[0.2em] border-b border-slate-800 pb-4">সেটিংস</h4>
                    <div className="space-y-5">
                        <textarea value={settings.marqueeNotice} onChange={e => setSettings({ ...settings, marqueeNotice: e.target.value })} className="w-full bg-slate-950 p-5 rounded-2xl border border-slate-800 text-xs font-black text-slate-300 outline-none focus:border-orange-500" rows={3} />
                        <div className="grid grid-cols-2 gap-4">
                            <input value={settings.adminBkash} onChange={e => setSettings({ ...settings, adminBkash: e.target.value })} className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-black text-white outline-none" placeholder="বিকাশ" />
                            <input value={settings.adminNagad} onChange={e => setSettings({ ...settings, adminNagad: e.target.value })} className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-black text-white outline-none" placeholder="নগদ" />
                        </div>
                        <button onClick={()=>alert('সেভ হয়েছে!')} className="w-full bg-orange-600 py-5 rounded-[24px] font-black text-[10px] uppercase text-white shadow-xl">সেভ করুন</button>
                    </div>
                </div>
            )}
        </div>
    ...
    return (
        <div className="space-y-6 pb-10">
            ...
            <div className="bg-slate-900 p-8 rounded-[40px] border border-slate-800 space-y-4 shadow-xl">
                <input value={newName} onChange={e => setNewName(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white outline-none focus:border-orange-500 shadow-inner" placeholder="নাম পরিবর্তন করুন" />
                <button onClick={() => { update(ref(db, 'users/' + currentUser.id), { name: newName }).then(() => alert('আপডেট হয়েছে!')); }} className="w-full bg-slate-800 py-4 rounded-2xl font-black text-[10px] uppercase text-white">আপডেট প্রোফাইল</button>
                <div className="pt-4 border-t border-slate-800 space-y-3">
                    <button onClick={() => navigate('/contact')} className="w-full bg-orange-600/10 text-orange-500 py-5 rounded-2xl font-black text-[10px] uppercase border border-orange-500/20 active:scale-95 transition-all">অ্যাডমিন সাপোর্ট (চ্যাট)</button>
    ...
    return (
        <div className="bg-slate-900 p-10 rounded-[48px] border border-slate-800 shadow-2xl mt-10 animate-in zoom-in duration-300">
            ...
            <form onSubmit={submit} className="space-y-5">
                {isReg && (<input placeholder="পুরো নাম" onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white shadow-inner outline-none focus:border-orange-500" required />)}
                <input placeholder="ফোন নাম্বার" onChange={e => setForm({ ...form, phone: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white shadow-inner outline-none focus:border-orange-500" required />
                <input type="password" placeholder="পাসওয়ার্ড" onChange={e => setForm({ ...form, password: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-xs font-black text-white shadow-inner outline-none focus:border-orange-500" required />
                <button className="w-full bg-orange-600 py-5 rounded-[24px] font-black text-sm text-white uppercase mt-4 shadow-xl active:scale-95 transition-all">{isReg ? 'রেজিস্ট্রেশন' : 'লগইন'}</button>
            </form>
            <p onClick={() => setIsReg(!isReg)} className="text-center text-[10px] text-slate-600 mt-10 cursor-pointer uppercase font-black tracking-widest underline">{isReg ? 'ইতিমধ্যেই আইডি আছে? লগইন করুন' : 'নতুন অ্যাকাউন্ট তৈরি করুন'}</p>
        </div>
    );
};
...
const Contact = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-250px)] bg-slate-900 rounded-[40px] border border-slate-800 overflow-hidden shadow-2xl">
            <div className="p-5 bg-slate-950 font-black text-[10px] uppercase text-orange-500 border-b border-slate-800 tracking-widest">সাপোর্ট চ্যাট</div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-950/20 scrollbar-hide">
                {chat.map((m: any) => (
                    <div key={m.id} className={`flex ${m.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] px-5 py-3 rounded-[24px] text-[11px] font-bold ${m.senderId === currentUser.id ? 'bg-orange-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-300 rounded-tl-none'}`}>{m.message}</div>
                    </div>
                ))}
                <div ref={scrollRef} />
            </div>
